...
for path in cell_paths:
    ...
    temp_props = (temp_props - temp_props.quantile(0.05)) / (
        temp_props.quantile(0.95) - temp_props.quantile(0.05)
    )
...
for path in cell_paths:
    ...
    if path[71] == "5":
        color = "tab:cyan"
    plt.plot(temp_props["ascini_position"], trend[prop], color=color)
    plt.title(prop)
    # plt.title(path[68:88])
    # plt.figure()
plt.plot(temp_props["ascini_position"], mean_df.mean(axis=1), color="k")
# %%
